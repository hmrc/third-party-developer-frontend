...
class ChangeApplicationDetailsSpec extends CommonViewSpec with WithCSRFAddToken {
  ...
  "change application details page" should {
    ...
    def renderPage(application: Application) = {
      ...
      changeDetails.render(form, ApplicationViewModel(application, hasSubscriptionsFields = false, hasPpnsFields = false, hasFraudPreventionHeaders = false), request, loggedIn, messagesProvider, appConfig, "nav-section")
    }
