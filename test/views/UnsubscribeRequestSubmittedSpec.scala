...
class UnsubscribeRequestSubmittedSpec extends UnitSpec with OneServerPerSuite with SharedMetricsClearDown with MockitoSugar {
  "Unsubscribe request submitted page" should {
    "render with no errors" in {
      ...
      val application = Application(appId, clientId, "Test Application", DateTimeUtils.now, DateTimeUtils.now, Environment.PRODUCTION, Some("Test Application Description"),
        Set(Collaborator(developer.email, Role.ADMINISTRATOR)), state = ApplicationState.production(developer.email, ""),
        access = Standard(redirectUris = Seq("https://red1", "https://red2"), termsAndConditionsUrl = Some("http://tnc-url.com")))

      val page = views.html.unsubscribeRequestSubmitted.render(ApplicationView(application, false), apiName, apiVersion, request, developer, applicationMessages, appConfig, "subscriptions")
