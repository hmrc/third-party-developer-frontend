...
class LeftHandNavSpec extends CommonViewSpec {
  ...
  "Left Hand Nav" should {
    ...
    "include links to client ID and client secrets if the user is an admin and the app has reached production state" in new Setup {
      val application = standardApplication.copy(collaborators = Set(Collaborator(loggedIn.email, CollaboratorRole.ADMINISTRATOR, UserId.random)), state = ApplicationState.production("", ""))
    ...
    "include links to client ID and client secrets if the user is not an admin but the app is in sandbox" in new Setup {
      val application =
        standardApplication.copy(deployedTo = Environment.SANDBOX, collaborators = Set(Collaborator(loggedIn.email, CollaboratorRole.DEVELOPER, UserId.random)), state = ApplicationState.production("", ""))
    ...
    "include link to push secrets when the application has PPNS fields and the user is an admin or the application is sandbox" in new Setup {
      val prodAppAsAdmin = standardApplication.copy(deployedTo = Environment.PRODUCTION, collaborators = Set(Collaborator(loggedIn.email, CollaboratorRole.ADMINISTRATOR, UserId.random)))
      val sandboxAppAsDev = standardApplication.copy(deployedTo = Environment.SANDBOX, collaborators = Set(Collaborator(loggedIn.email, CollaboratorRole.DEVELOPER, UserId.random)))
    ...
    "not include link to push secrets when the application does not have PPNS fields, or it does, but the user is only a developer for a production app" in new Setup {
      val prodAppAsAdmin = standardApplication.copy(deployedTo = Environment.PRODUCTION, collaborators = Set(Collaborator(loggedIn.email, CollaboratorRole.ADMINISTRATOR, UserId.random)))
      val sandboxAppAsDev = standardApplication.copy(deployedTo = Environment.SANDBOX, collaborators = Set(Collaborator(loggedIn.email, CollaboratorRole.DEVELOPER, UserId.random)))
      val prodAppAsDev = standardApplication.copy(deployedTo = Environment.PRODUCTION, collaborators = Set(Collaborator(loggedIn.email, CollaboratorRole.DEVELOPER, UserId.random)))
