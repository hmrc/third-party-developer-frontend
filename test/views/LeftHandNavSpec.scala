...
class LeftHandNavSpec extends CommonViewSpec with CollaboratorTracker with LocalUserIdTracker {
  ...
  "Left Hand Nav" should {
    ...
    "include links to manage API subscriptions, credentials and team members for an app with standard access" in new Setup {
      val document: Document = Jsoup.parse(leftHandNavView(Some(createApplicationViewModel(standardApplication, hasSubscriptionsFields = false, hasPpnsFields = false)), Some("")).body)
    ...
    "include links to manage team members but not API subscriptions for an app with privileged access" in new Setup {
      val document: Document = Jsoup.parse(leftHandNavView(Some(createApplicationViewModel(privilegedApplication, hasSubscriptionsFields = false, hasPpnsFields = false)), Some("")).body)
    ...
    "include links to manage team members but not API subscriptions for an app with ROPC access" in new Setup {
      val document: Document = Jsoup.parse(leftHandNavView(Some(createApplicationViewModel(ropcApplication, hasSubscriptionsFields = false, hasPpnsFields = false)), Some("")).body)
    ...
    "include links to client ID and client secrets if the user is an admin and the app has reached production state" in new Setup {
      ...
      val document: Document = Jsoup.parse(leftHandNavView(Some(createApplicationViewModel(application, hasSubscriptionsFields = false, hasPpnsFields = false)), Some("")).body)
    ...
    "include link to push secrets when the application has PPNS fields and the user is an admin or the application is sandbox" in new Setup {
      ...
      val prodAppAsAdminDocument: Document = Jsoup.parse(leftHandNavView(Some(createApplicationViewModel(prodAppAsAdmin, hasSubscriptionsFields = true, hasPpnsFields = true)), Some("")).body)
      val sandboxAppAsDevDocument: Document = Jsoup.parse(leftHandNavView(Some(createApplicationViewModel(sandboxAppAsDev, hasSubscriptionsFields = true, hasPpnsFields = true)), Some("")).body)
    ...
    "not include link to push secrets when the application does not have PPNS fields, or it does, but the user is only a developer for a production app" in new Setup {
      ...
      val prodAppAsAdminDocument: Document = Jsoup.parse(leftHandNavView(Some(createApplicationViewModel(prodAppAsAdmin, hasSubscriptionsFields = true, hasPpnsFields = false)), Some("")).body)
      val sandboxAppAsDevDocument: Document = Jsoup.parse(leftHandNavView(Some(createApplicationViewModel(sandboxAppAsDev, hasSubscriptionsFields = true, hasPpnsFields = false)), Some("")).body)
      val prodAppAsDevDocument: Document = Jsoup.parse(leftHandNavView(Some(createApplicationViewModel(prodAppAsDev, hasSubscriptionsFields = true, hasPpnsFields = true)), Some("")).body)
