...
class ThirdPartyDeveloperConnectorIntegrationSpec extends BaseConnectorIntegrationSpec with GuiceOneAppPerSuite {
  ...
  "fetchSession" should {
    ...
    "return the session" in new Setup {
      ...
      result shouldBe Session(sessionId, Developer(userEmail, "John", "Doe", loggedInState = LoggedInState.LOGGED_IN), LoggedInState.LOGGED_IN)
    }
  ...
  "authenticate" should {
    ...
    "return the session containing the user when the credentials are valid and MFA is disabled" in new Setup {
      ...
      result shouldBe UserAuthenticationResponse(
        accessCodeRequired = false,
        session = Some(Session(sessionId, Developer(userEmail, "John", "Doe", loggedInState = LoggedInState.LOGGED_IN), LoggedInState.LOGGED_IN)))
    }
