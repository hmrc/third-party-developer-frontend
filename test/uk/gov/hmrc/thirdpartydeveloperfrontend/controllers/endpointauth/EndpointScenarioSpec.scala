...
import uk.gov.hmrc.apiplatform.modules.applications.core.interface.models.ApplicationNameValidationResult.ValidApplicationName
...
abstract class EndpointScenarioSpec extends AsyncHmrcSpec with GuiceOneAppPerSuite with Writeables
  ...
  when(tpoConnector.validateName(*[String], *[Option[ApplicationId]], eqTo(Environment.PRODUCTION))(*)).thenReturn(Future.successful(Valid))
  ...
  when(tpoConnector.validateName(*[String], *[Option[ApplicationId]], eqTo(Environment.SANDBOX))(*)).thenReturn(Future.successful(ApplicationNameValidation(ValidApplicationName)))
