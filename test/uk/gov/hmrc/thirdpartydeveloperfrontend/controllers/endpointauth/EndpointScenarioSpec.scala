...
abstract class EndpointScenarioSpec extends AsyncHmrcSpec with GuiceOneAppPerSuite with Writeables
  ...
  final def getBodyParameterValues(endpoint: Endpoint): Map[String,String] = {
    endpoint match {
      ...
      case Endpoint("POST", "/developer/profile/email-preferences/topics") => Map("currentpassword" -> userPassword, "password" -> (userPassword + "new"), "confirmpassword" -> (userPassword + "new"))
