...
abstract class EndpointScenarioSpec extends AsyncHmrcSpec with GuiceOneAppPerSuite with Writeables
  ...
  when(apmConnector.updateApplication(*[ApplicationId],*[ApplicationUpdate])(*)).thenReturn(Future.successful(application))
  when(apmConnector.subscribeToApi(*[ApplicationId],*[SubscribeToApi])(*)).thenReturn(Future.successful(ApplicationUpdateSuccessful))
  ...
  def getEndpointSuccessResponse(endpoint: Endpoint): Response = {
    endpoint match {
      ...
      case Endpoint("POST", "/developer/applications/:id/client-secret-new") => Success()
