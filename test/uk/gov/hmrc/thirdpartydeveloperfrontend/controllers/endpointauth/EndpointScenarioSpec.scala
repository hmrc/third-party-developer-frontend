...
abstract class EndpointScenarioSpec extends AsyncHmrcSpec with GuiceOneAppPerSuite with Writeables
  ...
  final def getQueryParameterValues(endpoint: Endpoint): Map[String,String] = {
    endpoint match {
      ...
      case Endpoint(_,      "/developer/login-mfa") => Map("mfaId" -> authAppMfaId.value.toString, "mfaType" -> MfaType.AUTHENTICATOR_APP.toString)
      ...
      case Endpoint("GET",  "/developer/profile/protect-account/access-code") => Map("mfaId" -> authAppMfaId.value.toString)
      case Endpoint("POST", "/developer/profile/protect-account/enable") => Map("mfaId" -> authAppMfaId.value.toString)
      case Endpoint(_,      "/developer/profile/protect-account/remove-by-id") => Map("mfaId" -> authAppMfaId.value.toString)
      ...
      case Endpoint("GET",  "/developer/profile/security-preferences/auth-app/access-code") => Map("mfaId" -> authAppMfaId.value.toString, "mfaAction" -> MfaAction.CREATE.toString, "mfaIdForRemoval" -> authAppMfaId.value.toString)
      case Endpoint("POST", "/developer/profile/security-preferences/auth-app/access-code") => Map("mfaId" -> authAppMfaId.value.toString, "mfaAction" -> MfaAction.CREATE.toString, "mfaIdForRemoval" -> authAppMfaId.value.toString)
  ...
  final def getBodyParameterValues(endpoint: Endpoint): Map[String,String] = {
    endpoint match {
      ...
      case Endpoint("POST", "/developer/login/select-mfa") => Map("mfaId" -> authAppMfaId.value.toString)
      case _ => Map.empty
    }
  }
  ...
  def getEndpointSuccessResponse(endpoint: Endpoint): Response = {
    endpoint match {
      ...
      case Endpoint("POST", "/developer/login-mfa") => Redirect("/developer/profile/security-preferences/sms/setup/reminder")
      ...
      case Endpoint("POST", "/developer/profile/security-preferences/auth-app/access-code") => Redirect(s"/developer/profile/security-preferences/auth-app/name?mfaId=${authAppMfaId.value.toString}")
