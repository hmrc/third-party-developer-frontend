...
abstract class EndpointScenarioSpec extends AsyncHmrcSpec with GuiceOneAppPerSuite with Writeables
  ...
  final def getQueryParameterValues(endpoint: Endpoint): Map[String,String] = {
    endpoint match {
      ...
      case Endpoint("GET",  "/developer/profile/security-preferences/auth-app/access-code") => Map("mfaId" -> mfaId.value.toString, "mfaAction" -> MfaAction.CREATE.toString, "mfaIdToRemove" -> mfaId.value.toString)
      case Endpoint("POST", "/developer/profile/security-preferences/auth-app/access-code") => Map("mfaId" -> mfaId.value.toString, "mfaAction" -> MfaAction.CREATE.toString, "mfaIdToRemove" -> mfaId.value.toString)
      ...
      case Endpoint("GET",  "/developer/profile/security-preferences/sms/access-code") => Map("mfaId" -> smsMfaId.value.toString, "mfaAction" -> MfaAction.CREATE.toString, "mfaIdToRemove" -> smsMfaId.value.toString)
      case Endpoint("POST",  "/developer/profile/security-preferences/sms/access-code") => Map("mfaId" -> smsMfaId.value.toString, "mfaAction" -> MfaAction.CREATE.toString, "mfaIdToRemove" -> smsMfaId.value.toString)
  ...
  def getEndpointSuccessResponse(endpoint: Endpoint): Response = {
    endpoint match {
      ...
      case Endpoint("GET", "/developer/profile/security-preferences/remove-mfa") => Redirect(s"/developer/profile/security-preferences/sms/access-code?mfaId=${smsMfaId.value.toString}&mfaAction=REMOVE&mfaIdToRemove=${smsMfaId.value.toString}")
