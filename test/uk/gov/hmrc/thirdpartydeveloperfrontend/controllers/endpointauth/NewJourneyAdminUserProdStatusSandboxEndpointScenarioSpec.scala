...
class NewJourneyAdminUserProdStatusSandboxEndpointScenarioSpec extends EndpointScenarioSpec
  ...
  override def getExpectedResponse(endpoint: Endpoint): Response = {
    endpoint match {
      ...
      case Endpoint(_, "/developer/applications/:id/details/change-tc-and-priv-pol-url", _)                  => getEndpointSuccessResponse(endpoint)
      ...
      case Endpoint("GET", "/developer/submissions/application/:aid/submit-request", _)                      =>
        Redirect(s"/developer/submissions/application/$applicationId/production-credentials-checklist")
      case Endpoint("POST", "/developer/applications/:id/change-app-name-and-desc", _)                       =>
        Redirect(s"/developer/applications/$applicationId/manage")
      case Endpoint("GET", "/developer/applications/:id/change-app-name-and-desc", _)                        => Success()
