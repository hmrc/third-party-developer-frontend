...
class OldJourneyAdminUserProdStatusSandboxEndpointScenarioSpec extends EndpointScenarioSpec
  ...
  override def getExpectedResponse(endpoint: Endpoint): Response = {
    endpoint match {
      ...
      case Endpoint("GET",  "/developer/applications/:id/add/subscription-configuration-step/:pageNumber", _) => Redirect(s"/developer/applications/${applicationId.value}/add/success")
