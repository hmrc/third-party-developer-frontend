...
trait ApplicationStateHelper {
  ...
  object InState {
    ...
    def pendingGatekeeperApproval(requestedByEmail: String, requestedByName: String) =
      ApplicationState(State.PendingGatekeeperApproval, Some(requestedByEmail), Some(requestedByName), None, instant)

    def pendingResponsibleIndividualVerification(requestedByEmail: String, requestedByName: String) =
      ApplicationState(State.PendingResponsibleIndividualVerification, Some(requestedByEmail), Some(requestedByName), None, instant)

    def pendingRequesterVerification(requestedByEmail: String, requestedByName: String, verificationCode: String) =
      ApplicationState(State.PendingRequesterVerification, Some(requestedByEmail), Some(requestedByName), Some(verificationCode), instant)

    def preProduction(requestedByEmail: String, requestedByName: String) =
      ApplicationState(State.PreProduction, Some(requestedByEmail), Some(requestedByName), None, instant)
