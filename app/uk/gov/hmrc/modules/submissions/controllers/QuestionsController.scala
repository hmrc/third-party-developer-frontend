...
import uk.gov.hmrc.modules.submissions.domain.services.SubmissionsJsonFormatters._
...
class QuestionsController @Inject()(
  ...
  def showQuestion(submissionId: SubmissionId, questionId: QuestionId, answers: Option[ActualAnswer] = None, errors: Option[String] = None) = withSubmission(submissionId) { implicit request => 
    ...
    (
      for {
        ...
        val submitAction: Call = uk.gov.hmrc.modules.submissions.controllers.routes.QuestionsController.recordAnswer(submissionId, questionId)
        errors.fold(
  ...
  def recordAnswer(submissionId: SubmissionId, questionId: QuestionId) = withSubmission(submissionId) { implicit request => 
    ...
    val success = (extSubmission: ExtendedSubmission) => { 
      ...
      lazy val toProdChecklist = uk.gov.hmrc.modules.submissions.controllers.routes.ProdCredsChecklistController.productionCredentialsChecklistPage(extSubmission.submission.applicationId)
      lazy val toNextQuestion = (nextQuestionId) => uk.gov.hmrc.modules.submissions.controllers.routes.QuestionsController.showQuestion(submissionId, nextQuestionId)
