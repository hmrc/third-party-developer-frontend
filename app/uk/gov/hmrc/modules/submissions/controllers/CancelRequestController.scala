  ...
  def cancelRequestForProductionCredentialsAction(appId: ApplicationId) = withApplicationSubmission(StateFilter.notProduction)(appId) { implicit request =>
    lazy val goBackToRegularPage =
      if(request.submissionRequest.extSubmission.isCompleted) {
        Redirect(uk.gov.hmrc.modules.submissions.controllers.routes.CheckAnswersController.checkAnswersPage(appId))
      } else {
        Redirect(uk.gov.hmrc.modules.submissions.controllers.routes.ProdCredsChecklistController.productionCredentialsChecklistPage(appId))
      }
